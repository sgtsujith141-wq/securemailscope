...
The **demonstration video is finished**: 1920x1080, 30 fps, H.264 CRF 16,
built from a Playwright recording of the real application driving the real
backend over the real forensic engine. It opens on a real finding rather than
a title, and shows the same finding, the same packets, the same score and the
same drift example the deck does.
